...
describe("ProjectReference", () => {
  it.live("uses the broad experimental flag unless references are explicitly configured", () =>
    withEnv(
      { KILO_EXPERIMENTAL: "true", KILO_EXPERIMENTAL_REFERENCES: undefined },
      Effect.sync(() => {
        expect(Flag.KILO_EXPERIMENTAL_REFERENCES).toBe(true)
      }),
    ).pipe(
      Effect.flatMap(() =>
        withEnv(
          { KILO_EXPERIMENTAL: "true", KILO_EXPERIMENTAL_REFERENCES: "false" },
          Effect.sync(() => {
            expect(Flag.KILO_EXPERIMENTAL_REFERENCES).toBe(false)
          }),
        ),
      ),
    ),
...
function withReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ KILO_EXPERIMENTAL_REFERENCES: "true" }, body)
}

function withoutReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ KILO_EXPERIMENTAL: undefined, KILO_EXPERIMENTAL_REFERENCES: undefined }, body)
}
